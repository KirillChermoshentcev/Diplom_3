...
class TestFeedPage:
    ...
    def test_new_order_from_history_is_displaying_in_order_feed_success(self, driver, login_user,
                                                                        create_order):
        driver = login_user
        main_page = MainPage(driver)
    ...
    @allure.title('Проверка, что после создания нового заказа, увеличивается счетчик общего количества'
                  'выполненных заказов')
    def test_changing_counter_of_quantity_orders_for_all_time_success(self, driver, login_user, create_order):
    ...
    def test_changing_counter_of_quantity_orders_for_today_success(self, driver, login_user, create_order):
    ...
    def test_number_of_new_order_is_displaying_in_progress_section(self, driver, login_user, create_order):
